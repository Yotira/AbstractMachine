<%--
  Created by IntelliJ IDEA.
  User: Administrator
  Date: 2016/6/16
  Time: 16:09
  To change this template use File | Settings | File Templates.
--%>
<%@ page contentType="text/html;charset=UTF-8" language="java" %>

<!DOCTYPE html>
<head>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <title>Le的计算过程 </title>
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta name="description" content="Free HTML5 Template by FREEHTML5.CO" />
    <meta name="keywords" content="free html5, free template, free bootstrap, html5, css3, mobile first, responsive" />
    <meta name="author" content="FREEHTML5.CO" />

    <!-- Theme Style -->
    <link rel="stylesheet" href="../../resources/css/style.css">
    <link rel="stylesheet" type="text/css" href="../../resources/css/page.css">
</head>
<body>
<main role="main" id="fh5co-main">
    <div class="title">
        <div class="container">语言L<sub>e</sub>的计算过程</div>
    </div>
    <div class="fh5co-section-seperator fh5co-section-seperator-white" id="contact-animate" style="background-image: url(../../resources/images/hero-4.jpg);" data-stellar-background-ratio="0.5">
        <div class="fh5co-overlay"></div>
        <div class="container">
            <div class="fh5co-section-content-wrap">
                <div class="fh5co-section-content">
                    <div class="row">

                        <div class="col-md-2">
                        </div>

                        <div class="col-md-10 col-md-push-1">
                            <div class="row">
                                <div class="col-md-5">
                                    <div class="form-group to-animate">
                                        <label for="name" class="sr-only">Control</label>
                                        <input type="text" id="name" class="form-control J_control" placeholder="Control">
                                    </div>
                                    <div class="form-group to-animate">
                                        <label class="sr-only">Email</label>
                                        <input type="text" class="form-control J_stack" placeholder="Stack">
                                    </div>
                                    <div class="form-group to-animate">
                                        <label class="sr-only">DEnv</label>
                                        <input type="text"class="form-control J_denv" placeholder="DEnv">
                                    </div>
                                    <div class="form-group">
                                        <input type="butten" class="btn btn-primary col-md-5 J_submit" hasSubmit="0" value="Begin">
                                        <input type="butten" class="btn btn-primary col-md-5 pull-right J_run" hasSubmit="0"value="Running">
                                    </div>
                                </div>
                                <div class="col-md-7">
                                    <div class="form-group to-animate">
                                        <label for="message" class="sr-only">message</label>
                                        <textarea name="" id="message" cols="30" rows="5" class="form-control J_list" placeholder=""></textarea>
                                    </div>
                                </div>
                                <%--<div class="col-md-6">--%>
                                    <%--<div class="form-group">--%>
                                        <%--<input type="butten" class="btn btn-primary col-md-5 J_submit" hasSubmit="0" value="Begin">--%>
                                        <%--<input type="butten" class="btn btn-primary col-md-5 pull-right J_run" hasSubmit="0"value="Running">--%>
                                    <%--</div>--%>
                                <%--</div>--%>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <!-- END .container -->

    <!-- jQuery -->
    <script src="../../resources/js/jquery.min.js"></script>
    <script src="../../resources/js/bootstrap.min.js"></script>
    <script type="text/javascript" src="../../resources/js/page.js"></script>
</body>
</html>
